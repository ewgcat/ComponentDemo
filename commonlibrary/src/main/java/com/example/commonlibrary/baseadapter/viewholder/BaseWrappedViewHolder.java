...
public class BaseWrappedViewHolder extends RecyclerView.ViewHolder {
    ...
    public BaseWrappedViewHolder setImageUrl(int id, String url) {
        if (getView(id) instanceof ImageView) {
            RequestOptions options = centerCropTransform().priority(Priority.HIGH).diskCacheStrategy(DiskCacheStrategy.RESOURCE);
            Glide.with(itemView.getContext()).load(url).apply(options).into((ImageView) getView(id));
        }
        return this;
    }
    ...
    public BaseWrappedViewHolder setImageUrl(int id, String url, int errorId, int placeHolderId) {
        if (getView(id) instanceof ImageView) {
            RequestOptions options = centerCropTransform().priority(Priority.HIGH).diskCacheStrategy(DiskCacheStrategy.RESOURCE).error(errorId).placeholder(placeHolderId);

...
public class DragPointView extends View {
    ...
    private void initBodyPoint() {
        ...
        //肩
        point_jian_left = new DragPointBean(centerX - 2 * (centerX - point_ear_left.getPointF().x), 2 * point_ear_left.getPointF().y, PointEnum.POINT_JIAN_LEFT.getValue());
        point_jian_right = new DragPointBean(centerX + (centerX - point_jian_left.getPointF().x), point_jian_left.getPointF().y, PointEnum.POINT_JIAN_RIGHT.getValue());

        //肚脐
        point_navel = new DragPointBean(centerX, centerY - point_ear_left.getPointF().y, PointEnum.POINT_NAVEL.getValue());

        //胯骨
        point_hipbone_left = new DragPointBean(point_ear_left.getPointF().x, centerY, PointEnum.POINT_HIPBONE_LEFT.getValue());
        point_hipbone_right = new DragPointBean(point_ear_right.getPointF().x, point_hipbone_left.getPointF().y, PointEnum.POINT_HIPBONE_RIGHT.getValue());

        //膝盖
        point_knee_left = new DragPointBean(point_hipbone_left.getPointF().x - 20, centerY + (centerY / 2 - 100), PointEnum.POINT_KNEE_LEFT.getValue());
        point_knee_right = new DragPointBean(centerX + (centerX - point_knee_left.getPointF().x), point_knee_left.getPointF().y, PointEnum.POINT_KNEE_RIGHT.getValue());

        //脚踝
        point_ankle_left = new DragPointBean(point_knee_left.getPointF().x + 30, point_knee_left.getPointF().y + (centerY / 2 - 100), PointEnum.POINT_ANKLE_LEFT.getValue());
        point_ankle_right = new DragPointBean(centerX + (centerX - point_ankle_left.getPointF().x), point_ankle_left.getPointF().y, PointEnum.POINT_ANKLE_RIGHT.getValue());

        //脚尖
        point_tiptoe_left = new DragPointBean(point_knee_left.getPointF().x, point_ankle_left.getPointF().y + (2 * centerY - point_ankle_left.getPointF().y - 50), PointEnum.POINT_TIPTOE_LEFT.getValue());
        point_tiptoe_right = new DragPointBean(centerX + (centerX - point_tiptoe_left.getPointF().x), point_tiptoe_left.getPointF().y, PointEnum.POINT_TIPTOE_RIGHT.getValue());
    ...
    public List<Double> returnOrientation() {
        ...
        if (point_knee_left.getPointF().x < point_ankle_left.getPointF().x) { // 公共点在两点左边
